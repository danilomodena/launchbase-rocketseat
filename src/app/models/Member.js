...
module.exports = {
    ...
    create(data, callback) {
        const query = `INSERT INTO members (
            avatar_url, name, email, birth, gender, weight, height, instructor_id)
    ...
    find(id, callback) {
        ...
        LEFT JOIN instructors ON (members.instructor_id = instructors.id) WHERE members.id=$1`, [id], function (err, results) {
            if (err) throw `Database error. ${err}`
            callback(results.rows[0])
        })
    },
    ...
    avatar_url=($1), name=($2), email=($3), birth=($4), gender=($5), weight=($6), height=($7), instructor_id=($8) WHERE id=($9)`
